...
- **Correcciones de calidad (2026-04-30):**
  ...
  - **i18n exhaustiva** — eliminadas todas las cadenas hardcodeadas en `not-found`, `public-profile`, `mini-card-page`, `admin-products`, `game-list` (meta tags) y `landing` (meta tags). El atributo `title="Verificado"` reemplazado por `[title]="'badges.verificado' | transloco"` en user-profile-card, user-public-profile, user-profile, user-followers y product-detail.
  - **Nuevas claves i18n** — añadidas en los 5 idiomas: `not_found.glitch`, `public_profile.unfollow_title`, `public_profile.unfollow_confirm`, `public_profile.not_found`, `common.cancel`, `common.loading` (con ellipsis), `badges.verificado`, `meta.games_title`, `meta.games_description`, `meta.landing_title`, `meta.landing_description`, `admin.products.field_developer`, `admin.products.field_publisher`.
  - **SafeUrlPipe reforzado** — valida que la URL sea de YouTube (`youtube.com/embed/` o `youtu.be/`, solo HTTPS) antes de llamar a `bypassSecurityTrustResourceUrl`. URLs inválidas devuelven cadena vacía saneada.
  - **HeaderComponent: setTimeout eliminado** — el mecanismo de hover-delay para el desplegable de géneros reemplazado por RxJS `timer` + `switchMap` + `takeUntilDestroyed`. El auto-cierre del modal de encuesta también usa `timer`.
  - **Formulario de reseña unificado** — `review-create` y `review-edit` fusionados en `src/app/features/review-form/ReviewFormComponent`. Detecta el modo (crear vs editar) consultando si la ruta tiene `:reviewId`. `app.routes.ts` actualizado para apuntar ambas rutas al nuevo componente.
